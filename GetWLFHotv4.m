function [File,Mastercurve,CWLF] = GetWLFHotv4(filename,Tref)
...
%%%%%%%%%%%%%%%%%%%%Lecture du fichier et Initialisation des variables%%%%%%%%%%%%%%%%%
...
Tabdel = DeltaShift(Mastercurve,CWLF,150,Tref);
...
Obj = DichotSolve(DataRef,DataMob(floor(MEST/2),3),[DataRef(1,1),DataRef(MEST,1)]);
disp(['Dichotomy done']);
Shft = Obj - DataMob(floor(MEST/2),1); %The shift is just the one to match the mid-range frequency point on the master curve already done.

%%%%Filling of the Mastercurve w/ modified frequencies%%%%
Datashft(floor(MEST/2),1) = DataMob(floor(MEST/2),1)+Shft; %New frequency
Datashft(floor(MEST/2),2) = DataMob(floor(MEST/2),2); %Temperature, Gpri, Gsec and damping factor untouched
Datashft(floor(MEST/2),3) = DataMob(floor(MEST/2),3);
Datashft(floor(MEST/2),4) = DataMob(floor(MEST/2),4);
Datashft(floor(MEST/2),5) = DataMob(floor(MEST/2),5);
Datashft(floor(MEST/2),6) = Datashft(floor(MEST/2),1)/DataMob(floor(MEST/2),1); %Shift-factor
Datashft(floor(MEST/2),7) = log(Datashft(floor(MEST/2),6));

disp(['log(Shift-factor) = ',num2str(Datashft(floor(MEST/2),7))]);


for i = [1:floor(MEST/2)-1,floor(MEST/2)+1:MEST]
	Datashft(i,1) = DataMob(i,1)*Datashft(floor(MEST/2),6);
	Datashft(i,2) = DataMob(i,2);
	Datashft(i,3) = DataMob(i,3);
	Datashft(i,4) = DataMob(i,4);
	Datashft(i,5) = DataMob(i,5);
	Datashft(i,6) = Datashft(floor(MEST/2),6);
